<template>
  <div class="product-chart">
    <div class="content">
      ...
      <div class="message">
        <div class="max-box">
          <div>0</div><div>{{themeData.key === 'fundOrder' ? data.fundNum : data.trustNum}}</div>
        </div>
        <div class="message-num" :style="`color: ${themeData.color}`">
          <span>{{currentData.count}}</span><span>只</span>
        </div>
      ...
      <div class="bottom">
        <div class="bottom-list1">
          <div>{{ currentData.orderNum }}</div>
          <div>排名</div>
        </div>
        <div class="bottom-list2">
          <div>{{ themeData.key === 'fundOrder' ? data.fundNum : data.trustNum }}</div>
...
export default {
  ...
  computed: {
    currentData() {
      ...
      return this.data[this.themeData.key];
    },
    productOptions() {
      return calcPieChart(this.themeData.linear, this.currentData);
    },
    themeData() {
